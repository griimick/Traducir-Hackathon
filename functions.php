   ...
   <?php function query_submit($i,$o,$p,$p1,$p2,$a,$s,$d)
   {
       global $connection;
      
       $i=mysqli_real_escape_string($connection,$i);       
       $o=mysqli_real_escape_string($connection,$o);
       
       $s=mysqli_real_escape_string($connection,$s);
       
       
       
             $query="Insert Into signup(first_name,last_name,DATE_OF_BIRTH,Email_id,username,passwords) ";
             $query.="VALUES('{$i}','{$o}','{$p}-{$p1}-{$p2}','{$a}','{$s}','{$d}')";
...
<?php $pass="";
function verify_client($username1)
{
   ...
   $username1=mysqli_real_escape_string($connection,$username1);
          ...
          $query1.="where username='{$username1}'";
